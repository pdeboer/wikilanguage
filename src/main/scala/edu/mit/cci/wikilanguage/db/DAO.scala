...
object DAO extends DAOQueryReturningType {
	...
	def getFilteredConnectionsByYear(year: Int) = {
		val p = typedQuery[PersonLink](
			"""
			  SELECT id, person_from, person_to FROM connections_norm WHERE ? BETWEEN year_from AND IFNULL(year_to, year_from+100) AND year_FROM IS NOT NULL
			""", s => s.setInt(1, year), r => PersonLink(r.getInt(1), r.getInt(2), r.getInt(3))
		)
		p
	}

	//96385,91791,277486,35937,116194,122809,53067,27772,277487,277491,268232,268341,268785,79778,268242,203823,162095,31775,34804,156365,109235,113586,42442,258688,15811,258701,258698,79903,108799,42205,113032,27462,258752,34706,108834,113963,53398,113364,41957,42599,113049,274068,274194,155717,155676,39427,268245,58909,38857,45944,34545,38541,108911,21618,50964,32339,34540,234442,25146,25036,108188,25147,32448,108109,140864,144057,144045,44663,163275,140972,107935,108510,143971,38984,108071,127399,44363,161548,273519,88494,25304,24821,116195,118232,32270,140649,40249,117973,27776,26085,60884,63552,32587,116474,31052,79879,41684,273247,88573,49391,64305,34875,163846,108189,245507,25148,27978,140751,256670,108238,36026,116196,263149,262957,44003,263187,64306,39715,64479,40063,27777,42179,41862,53334,34314,50334,35131,42377,161327,30359,258823,46835,51212,64515,247724,256312,274120,46371,46484,212671,27556,114798,155685,263072,29266,35061,112929,1395,263181,29232,25528,53287,26784,36211,39425,51079,278233,274367,117218
